...
export class InvoiceCreateEditComponent implements OnInit, OnDestroy {
  ...
  initializeCreate(): void {
    this.formGroup = new FormGroup({
      type: new FormControl(this.typesOptions[0].value, [Validators.required]),
  ...
  initializeEdit(): void {
    this.selectedBuyer = this.invoice.buyer;
    this.formGroup = new FormGroup({
      type: new FormControl(this.invoice.type, [Validators.required]),
      number: new FormControl(this.invoice.number, [Validators.required]),
      dateOfCreate: new FormControl(this.invoice.dateOfCreate, [
        Validators.required,
      ]),
      dateOfTurnover: new FormControl(this.invoice.dateOfTurnover, [
        Validators.required,
      ]),
      dateOfMaturity: new FormControl(this.invoice.dateOfMaturity, [
        Validators.required,
      ]),
      placeOfIssue: new FormControl(this.invoice.placeOfIssue, [
        Validators.required,
      ]),
      netAmount: new FormControl(this.invoice.netAmount, [Validators.required]),
      vatAmount: new FormControl(this.invoice.vatAmount, [Validators.required]),
      grossAmount: new FormControl(this.invoice.grossAmount, [
        Validators.required,
      ]),
      currency: new FormControl(this.invoice.currency, [Validators.required]),
      country: new FormControl(this.invoice.country, [Validators.required]),
    ...
    if (this.formGroup.get('type')?.value === 'CASH') {
      this.formGroup.addControl(
        'numberOfCashBill',
        new FormControl(this.invoice.numberOfCashBill, [Validators.required])
      );
    }
  ...
  calculateInvoiceItemAmount(index: number, formControlName: string): void {
    ...
    switch (formControlName) {
      case 'netPrice':
        vatAmountControl?.setValue(
          (netPriceControl?.value * vatRateControl?.value) / 100
        );
        grossPriceControl?.setValue(
          netPriceControl?.value + vatAmountControl?.value
        );
        break;
      case 'vatRate':
        vatAmountControl?.setValue(
        ...
        break;
      case 'vatAmount':
        netPriceControl?.setValue(
          (vatAmountControl?.value * 100) / vatRateControl?.value
        );
        grossPriceControl?.setValue(
          netPriceControl?.value + vatAmountControl?.value
        );
      ...
      case 'grossPrice':
        vatAmountControl?.setValue(
          (grossPriceControl?.value * vatRateControl?.value) /
            (100 + vatRateControl?.value)
        );
        netPriceControl?.setValue(
          grossPriceControl?.value - vatAmountControl?.value
        );
        break;
    }
    this.calculateInvoiceAmount();
  ...
  setAllInvoiceAmounts(): void {
    this.invoiceItemsFormArr.controls.forEach((element) => {
      element
        .get('grossPrice')
        ?.setValue(roundOnDigits(element.get('grossPrice')?.value));
      element
        .get('vatAmount')
        ?.setValue(
          roundOnDigits(
            (element.get('grossPrice')?.value * element.get('vatRate')?.value) /
              (100 + element.get('vatRate')?.value)
          )
        );
      element
        .get('netPrice')
        ?.setValue(
          roundOnDigits(
            element.get('grossPrice')?.value - element.get('vatAmount')?.value
          )
        );
    });
    this.calculateInvoiceAmount();
