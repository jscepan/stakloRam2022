...
public class WorkOrderStore extends ObjectStore {
    ...
    public long getLastWorkOrderNumber(int year) throws SQLException {
        ...
        ResultSet resultSet = st.executeQuery("SELECT * from " + ConnectionToDatabase.DATABASE_NAME + "." + this.tableName + " WHERE " + this.getPrimaryKey() + "=(SELECT MAX(work_order_number) FROM " + ConnectionToDatabase.DATABASE_NAME + "." + this.tableName + " WHERE year(work_order_date_of_create)=" + year + ")");
